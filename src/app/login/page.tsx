...
export default function LoginPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(formatApiError(data, "Sign-in failed"));
        setPending(false);
        return;
      }
      router.push("/");
    ...
    <main className="min-h-full bg-[#050507] px-4 py-16 text-[#f3f4f6]">
      <div className="mx-auto w-full max-w-md rounded-2xl border border-[#32374380] bg-[#171a1f] p-8 shadow-xl">
        ...
        <p className="mt-2 text-sm text-[#bdc1ca]">
          The{" "}
          <code className="text-[#c4b5fd]">access_token</code> cookie is set by
          the Route Handler (httpOnly); tokens are not stored in browser
          storage.
        </p>
